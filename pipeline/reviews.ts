...
export interface Review {
   ...
   * 개정을 몇 번 시도했는가.
   *
   * 없으면 더 안 오르는 기사가 목록에 영원히 남아, 30분마다 같은 글을 다시 쓰느라
   * 호출만 태운다. 실제로 4.4 에서 멈춘 기사들이 그러고 있었다.
   * 날조가 남은 기사는 이 한도와 무관하게 계속 시도한다 — 거기서 멈추면
   * 사이트에 지어낸 수치가 남는다(그건 격리로 내려간다).
   */
  improveAttempts?: number;
}
